...
int main(int argc, char** argv)
{
  ...
  unsigned long timeToSleep = 5; //default sleep time for sockets.
  ...
  /***************************************************************************/
	...
	while (!reader->OKToRead() && (reconnectAttempts == 0 || tries < reconnectAttempts)) {
		sleep(timeToSleep);
		tries++;
		delete reader;
  ...
  ORDataProcManager dataProcManager(reader);
	...
	ORSNOPackedEventProcessor packedEventProcessor(label);
	...
	if (runAsDaemon) {
		...
		dataProcManager.AddProcessor(&packedEventProcessor);
	}
